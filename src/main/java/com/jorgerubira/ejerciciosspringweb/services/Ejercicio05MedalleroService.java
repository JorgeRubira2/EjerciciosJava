...
public class Ejercicio05MedalleroService implements IEjercicio05MedalleroService {
    ...
    public Ejercicio05MedalleroService() {
        ...
        medallas.add(new Medalla("Bélgica", "Maratón masculino", "Bronce", "Atletismo", "B. Abdi"));
        ...
        medallas.add(new Medalla("Ucrania", "Nado sincronizado femenino", "Bronce", "Natacion", "A. Savchuk, K. Sydorenko"));
        ...
        medallas.add(new Medalla("Gran Bretaña", "Clavados masculino", "Bronce", "Natacion", "T. Daley"));
    }
    ...
    public List<MedallaPais> obtenerRankingPorPais() {
        ...
        List<MedallaPais> res = paises.stream()
                ...
                (int) medallas.stream()
                        ...
                        .count(),
                (int) medallas.stream()
                        .filter(y -> y.getMedalla().equals("Bronce"))
                        .filter(y -> y.getPais().equals(x))
                        .count()
        ))
                .sorted((x, y) -> (y.getOro() + y.getPlata() + y.getBronce()) - (x.getOro() + x.getPlata() + x.getBronce()))
                .collect(Collectors.toList());
        return res;
    ...
    public List<MedallaAtleta> obtenerRankingPorAlteta() {
        ...
        List<String> deportistas = medallas.stream()
                .map(x -> x.getDeportistas())
                .distinct()
                .collect(Collectors.toList());

        List<MedallaAtleta> paso1 = deportistas.stream()
                .map(x -> new MedallaAtleta(x, 0, 0, 0))
                .collect(Collectors.toList());

        paso1.forEach(x -> {
            x.setOro((int) medallas.stream()
                    .filter(y -> y.getMedalla().equals("Oro"))
                    .filter(y -> y.getDeportistas().equals(x))
                    .count()
            );
        });

        List<MedallaAtleta> resultado = deportistas.stream()
