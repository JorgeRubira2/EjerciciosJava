...
public class Ejercicio06Lambdas {
    ...
    public Consumer<Persona> incrementarEdad() {
        return (persona) -> persona.setEdad(persona.getEdad() + 1);
    }

    /**
     * Crear una función BiConsumer que copie la compra de la segunda persona a
     * la primera. La cesta de la segunda persona debe quedar empty.
     */
    ...
    public IComparadorPersonaCompra miCompraEsMayorQueOtra() {
        return (persona, compra) -> {
            if (persona.getCesta().isEmpty()) {
                persona.setCesta(new Compra(0, false));
            }
            ...
            return persona.getCesta().get().getTotalArticulos() > compra.get().getTotalArticulos() ? true : false;
        };
    }
    ...
    public IComparadorPersonaCompra igualNumeroDeUnidades() {
        return (persona, compra) -> {
            if(persona.getCesta().isEmpty() && compra.isEmpty()){
                return true;
            }else if(persona.getCesta().isEmpty() || compra.isEmpty()){
                return false;
            }
            return persona.getCesta().get().getTotalArticulos() == compra.get().getTotalArticulos();
        };
    }
