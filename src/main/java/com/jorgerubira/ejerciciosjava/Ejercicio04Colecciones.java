...
public class Ejercicio04Colecciones {
    ...
    /**
     * Borrar todas las personas de la Lista que sean de Huesca.
     * No hace falta verificar si valen nulo.
     */
    ...
    /**
     * Borrar todas las personas del Map que sean de Huesca.
     * No hace falta verificar si valen nulo.
     * 
     */
    public void borrarPersonasHuescaDeMapa(Map<String, Persona> listaPersonas){
        Set<String> borrar= new HashSet<>();
        for(String val : listaPersonas.keySet()){
            if(listaPersonas.get(val).getCiudad().equals("Huesca"))
                borrar.add(val);
        }
        for(String b : borrar){
            listaPersonas.remove(b);
        }
    }
