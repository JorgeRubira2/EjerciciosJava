...
public class Ejercicio07StreamsNivelDificil {
    ...
    public Set<Persona> personaEnDosGrupos(Set<Persona> personasGradoMedio, Set<Persona> personasUniversidad, Set<Persona> personasCertificado){
        ...
        personasGradoMedio.stream().filter(x->{boolean x1=personasUniversidad.stream().anyMatch(y->x.getNombre().equals(y.getNombre()));
                                               boolean x2=personasCertificado.stream().anyMatch(y->x.getNombre().equals(y.getNombre()));
                                                return x1 || x2;}).forEach(x->resultado.add(new Persona(x.getNombre())));
        personasUniversidad.stream().filter(x->{boolean x1=personasGradoMedio.stream().anyMatch(y->x.getNombre().equals(y.getNombre()));
                                               boolean x2=personasCertificado.stream().anyMatch(y->x.getNombre().equals(y.getNombre()));
                                                return x1 || x2;}).forEach(x->resultado.add(new Persona(x.getNombre())));
        
        resultado.forEach(x->System.out.println(x.getNombre()));
        return resultado;
