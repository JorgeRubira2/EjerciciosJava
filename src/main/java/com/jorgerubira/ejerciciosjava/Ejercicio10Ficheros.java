...
public class Ejercicio10Ficheros {
    ...
    private final String rutaBase = "C:\\Users\\Sofia\\Desktop\\CursoJavaHiberus\\EjerciciosJava\\src\\main\\java\\com\\jorgerubira\\resources\\";
    ...
    public int contarCuantosEventosHay() {
        ...
        try {
            long elementos = Files.lines(Path.of(rutaBase, fichero), Charset.forName("ISO-8859-1")).count() - 1;
    ...
    public String buscarId() {
        ...
        try {
            return Files.lines(Path.of(rutaBase, fichero), Charset.forName("ISO-8859-1")).filter(x -> x.contains("Camino a Mercedes")).map(x -> x.split(";")[0]).findFirst().get();
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public int contarCoincidencias(String fichero, String palabra) {
        ...
        try {
            ...
            String contenido = Files.readString(Paths.get(rutaBase + fichero)).toLowerCase();
            int p = contenido.indexOf(palabra);

            while (p >= 0) {
                contador++;
                p = contenido.indexOf(palabra, p + 1);
            }
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
    ...
    public Optional<Double> calcularPromedio() {
        ...
        try {
            OptionalDouble media = Files.lines(Paths.get(rutaBase + fichero), Charset.forName("ISO-8859-1"))
    ...
    public List<Persona> cargarPersona() throws FileNotFoundException {
        ...
        try{
            List<String> elementos = Files.lines(Paths.get(rutaBase + fichero))
                    .skip(1)
                    .filter(x -> x.length() > 0)
                    .map(x -> (x.split(";")[0]))
                    .collect(Collectors.toList());
            persona = elementos.stream().map(x -> new Persona(x)).collect(Collectors.toList());
